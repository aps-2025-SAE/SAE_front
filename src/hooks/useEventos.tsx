        ...
        response.data.map(
          (evento) =>
            ({
              id: evento.id,
              title: evento.tipo,
              budget: isNaN(parseFloat(evento.valor))
                ? 0
                : Math.max(0, parseFloat(evento.valor)),
              description: evento.descricao,
              dateInit: addHourToDate(evento.data_inicio),
              dateEnd: addHourToDate(evento.data_fim),
              diaryOffers: isNaN(parseInt(evento.numOfertasDiarias))
                ? 0
                : Math.max(0, parseInt(evento.numOfertasDiarias)),
            } as Event)
        )
      );
    } catch (error) {
  ...
  const updateEvent = async (id: string, event: Event): Promise<boolean> => {
    ...
    try {
      const response = await axios.put<Event>(
        `https://saeback-production-fe02.up.railway.app/api/eventos/${id}`,
        {
          tipo: event.title,
      ...
      showMessage(
        "Erro ao atualizar evento. Verifique os dados e tente novamente.",
        "error"
      );
      return false;
    } finally {
